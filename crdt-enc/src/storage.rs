...
/// Where/how `Core` persists and lists the (already-protected) byte blobs it produces: local
/// per-device metadata, remote metadata gossiped between devices, full-state snapshots, and
...
{
    /// Called once by `Core::open`, concurrently with `Protector::init`, before the initial
    /// remote-meta read. `core` is a `dyn`-compatible handle back into the owning `Core`. The
    /// default no-op is enough for storage backends that don't need any setup.
    ...
    /// `Core`) changes, with the latest merged register -- `None` if no storage metadata has ever
    /// been written yet. Implementations that don't need any out-of-band metadata can rely on the
    /// default no-op.
    fn set_remote_meta(
        &self,
    ...
    /// Loads this device's own local metadata (e.g. `LocalMeta`), if any has been stored yet.
    /// Unlike remote meta/states/ops, this is device-local and never synced.
    ...
    /// published one), for `load_remote_metas` to load and `Core` to merge.
    ...
    /// Writes a new immutable full-state snapshot (produced by `Core::compact`) and returns the
    ...
    /// version with no gaps -- `Core` relies on this contiguous ordering to replay ops correctly.
    fn load_ops(
        &self,
        actor_first_versions: Vec<(Uuid, u64)>,
    ) -> impl Future<Output = Result<Vec<(Uuid, u64, VersionBytes)>>> + Send;

    /// Writes a new immutable op blob for `actor` at `version`. Callers (i.e. `Core`, for the local
    /// actor) are responsible for calling this with strictly ascending, contiguous versions per
    /// actor.
    fn store_ops(
        &self,
        actor: Uuid,
